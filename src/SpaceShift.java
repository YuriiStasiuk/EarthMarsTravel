public class SpaceShift {
	...
	static double vy;
	static double phi;
	public SpaceShift(Date date, double accelaration,double v, int planet) {
		...
		if(planet==1) {
			...
			x = (r * Math.cos(phi));
			y = (r * Math.sin(phi));
			SpaceShift.velocitiesMars();
		}
	}
	public static void velocitiesEarth() {
		double x0 = Earth.x;
		double y0 = Earth.y;
	...
	public static void velocitiesMars() {
		double x0 = Mars.x;
		double y0 = Mars.y;
		double r1 = (a*Math.sqrt(1-e*e))/(1 + e*Math.cos(24-Mars.phi-0.1));
		double x1 = r1 * Math.cos(phi+0.1);
		double y1 = r1 * Math.sin(phi+0.1);
		double deltaX = x0 - x1;
		double deltaY = y0 - y1;
	...
	public static void nextStepToEarth(int step) {
		double axSun = (G*mSun*x)/(Math.pow(x*x+y*y, 3/2));
		double axEarth = (G*mSun*(x-Earth.x))/(Math.pow((x-Earth.x)*(x-Earth.x)+(y-Earth.y)*(y-Earth.y), 3/2));
		double axMars = (G*mSun*(x-Mars.x))/(Math.pow((x-Mars.x)*(x-Mars.x)+(y-Mars.y)*(y-Mars.y), 3/2));
		double axEngine = -acceleration*(x-Earth.x)/(Math.pow((x-Earth.x)*(x-Earth.x)+(y-Earth.y)*(y-Earth.y), 1/2));
		double aySun = (G*mSun*y)/(Math.pow(x*x+y*y, 3/2));
		double ayEarth = (G*mSun*(y-Earth.y))/(Math.pow((x-Earth.x)*(x-Earth.x)+(y-Earth.y)*(y-Earth.y), 3/2));
		double ayMars = (G*mSun*(y-Mars.y))/(Math.pow((x-Mars.x)*(x-Mars.x)+(y-Mars.y)*(y-Mars.y), 3/2));
		double ayEngine = -acceleration*(y-Earth.y)/(Math.pow((x-Earth.x)*(x-Earth.x)+(y-Earth.y)*(y-Earth.y), 1/2));
		double ax = -axSun - axEarth - axMars + axEngine;
		double ay = -aySun - ayEarth - ayMars + ayEngine;
		x = x + vx*step + ax*step*step/2;
		y = y + vy*step + ay*step*step/2;
	...
	public static void nextStepTangentAgainstMovement(int step) {
		double axSun = (G*mSun*x)/(Math.pow(x*x+y*y, 3/2));
		double axEarth = (G*mSun*(x-Earth.x))/(Math.pow((x-Earth.x)*(x-Earth.x)+(y-Earth.y)*(y-Earth.y), 3/2));
		double axMars = (G*mSun*(x-Mars.x))/(Math.pow((x-Mars.x)*(x-Mars.x)+(y-Mars.y)*(y-Mars.y), 3/2));
		double axEngine = -acceleration*y/(Math.pow(x*x+y*y, 1/2));
		double aySun = (G*mSun*y)/(Math.pow(x*x+y*y, 3/2));
		double ayEarth = (G*mSun*(y-Earth.y))/(Math.pow((x-Earth.x)*(x-Earth.x)+(y-Earth.y)*(y-Earth.y), 3/2));
		double ayMars = (G*mSun*(y-Mars.y))/(Math.pow((x-Mars.x)*(x-Mars.x)+(y-Mars.y)*(y-Mars.y), 3/2));
		double ayEngine = acceleration*x/(Math.pow(x*x+y*y, 1/2));
		double ax = -axSun - axEarth - axMars + axEngine;
		double ay = -aySun - ayEarth - ayMars + ayEngine;
		x = x + vx*step + ax*step*step/2;
		y = y + vy*step + ay*step*step/2;
